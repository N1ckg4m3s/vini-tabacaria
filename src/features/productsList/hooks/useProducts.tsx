...
export const useProductsList: useProductsListProps = ({ numeroPorPagina, paginaAtual }) => {
    const [products, setProducts] = useState<Produto[]>([])
    const [totalPages, setTotalPages] = useState<number>(0)

    useEffect(() => {
        const asyncFunction = async () => {
            const productResponse = await fetchProductsPaginated({ filtros: {}, limit: numeroPorPagina, paginaAtual })
